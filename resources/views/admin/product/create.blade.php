          ...
          <div class="card">
            <div class="login-box">
              <form action="{{ route('admin.product.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="user-box mt-2">
                  <input type="number" name="price" id="price" class="@error('price') is-invalid @enderror" value="{{ old("price") }}" min="0" />
                  @error('price')
                    <div class="invalid-feedback">
                      {{ $message }}
                    </div>
                ...
                <div class="user-box mt-2">
                  <input type="number" name="stock" id="stock" class="@error('stock') is-invalid @enderror" min="0" value="{{ old("stock") }}"/>
                  @error('stock')
                    <div class="invalid-feedback">
                      {{ $message }}
                    </div>
                  ...
                  <div class="row">
                    <div class="col-6 px-0">
                      ...
                      <select name="product_category" class="form-select form-select-sm @error('product_category') is-invalid @enderror"
                        ...
                        <option value="catering" {{ old('product_category') == 'catering' ? 'selected' : '' }}>Catering</option>
                        <option value="frozen" {{ old('product_category') == 'frozen' ? 'selected' : '' }}>Frozen</option>
                      </select>
